...
%%% Access to the unicode data base (content of UnicodeData.txt,
%%% see "unidata.hrl" and unicode_data module).  These functions
%%% are just wrappers for ucs_data module functions.
-export([unicode_data/1, rebuild_database/0, rebuild_database/1]).

%%% Conversion to/from IANA recognised character sets
-export([to_unicode/2, from_unicode/2]).

%%% MIBenum (IANA definitions) encoding/decoding
-export([getMIB/1, getCharset/1]).
...
-export([to_mnemonic/1, to_mnemonic/2, from_mnemonic/1, from_mnemonic/2]).
...
%%% The following are exported only for the ucs_data module.
-export([code_to_string/1]).

-include("ucs_data.hrl").

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Force a start of the ucs server, unless already started.
start_server() ->
    ucs_data:ensure_started().

%%% Return struct containing full unicode data for Char.
unicode_data(Char) ->
    ucs_data:unicode_data(Char).


%%% Rebuild the database files:
%%%	unidata:   UnicodeData.txt -> unidata.dets
%%%	mnemonics: ucsmnem.edat -> ucsmnem1.dets,ucsmnem2.dets
%%%     mibenum:   IANA Charset numbers
%%%     all:       All of the above (default)
rebuild_database() ->
    ucs_data:rebuild_database().

rebuild_database([Which]) ->
    %% Can be wrapped in a list to support "erl -s ucs rebuild_database Which"
    ucs_data:rebuild_database(Which);
rebuild_database(Which) ->
    ucs_data:rebuild_database(Which).
...
%%% ............................................................................
%%% RFC-1345 style mnemonic string encoding and decoding
to_mnemonic(X) -> to_mnemonic(X,fun is_visible_ascii/1).

to_mnemonic(List,Pred) when is_list(List) ->
    lists:flatmap(fun(X) -> to_mnemonic(X,Pred) end, List);
to_mnemonic($&,_) ->
    "&&";
to_mnemonic(Ch,Pred) ->
    case Pred(Ch) of
	true -> [Ch];
	false ->
	    case (is_iso646_basic(Ch) and (Ch =/= $\s)) of
		true -> [Ch];
		false ->
		    case char_to_mnemonic(Ch) of
			Mnem=[_,_] -> [$&|Mnem];
			Mnem=[_,_|_] -> [$&,$_|Mnem++"_"]
		    end
	    end
...
char_to_mnemonic(Ch) ->
    case ucs_data:code_to_mnemonic(Ch) of
	undefined ->
	    "?u" ++ code_to_string(Ch);
	Mnem ->
	    Mnem
...
code_to_string(Ch) when is_integer(Ch), Ch >= 0 ->
    format_code(Ch,0,[]).
...
hex_to_integer([Digit|Digits],N) when is_integer(Digit) ->
    if Digit >= $0, Digit =< $9 ->
...
%%% ............................................................................
to_gsm0338(Input) when is_list(Input) ->
    to_gsm0338(Input,[]);
to_gsm0338(Input) when is_binary(Input) ->
    to_gsm0338(binary_to_list(Input)).

to_gsm0338([],Out) ->
    lists:reverse(Out);
to_gsm0338([915|Rest],Out) ->    to_gsm0338(Rest,[19|Out]);
to_gsm0338([916|Rest],Out) ->    to_gsm0338(Rest,[16|Out]);
to_gsm0338([920|Rest],Out) ->    to_gsm0338(Rest,[25|Out]);
to_gsm0338([923|Rest],Out) ->    to_gsm0338(Rest,[20|Out]);
to_gsm0338([926|Rest],Out) ->    to_gsm0338(Rest,[26|Out]);
to_gsm0338([928|Rest],Out) ->    to_gsm0338(Rest,[22|Out]);
to_gsm0338([931|Rest],Out) ->    to_gsm0338(Rest,[24|Out]);
to_gsm0338([934|Rest],Out) ->    to_gsm0338(Rest,[18|Out]);
to_gsm0338([936|Rest],Out) ->    to_gsm0338(Rest,[23|Out]);
to_gsm0338([937|Rest],Out) ->    to_gsm0338(Rest,[21|Out]);
to_gsm0338([8364|Rest],Out) ->   to_gsm0338(Rest,[7013|Out]);
to_gsm0338([Char|Rest],Out) ->
    case element(Char,?UNICODE_TO_GSM_0338) of
	undefined ->
	    {error,bad_character_code};
	Val ->
	    to_gsm0338(Rest,[Val|Out])
    end.
...
from_unicode(Input,Cs) when Cs=='ansi_x3.4-1968';Cs=='iso-ir-6';
			  Cs=='ansi_x3.4-1986';Cs=='iso_646.irv:1991';
			  Cs=='ascii';Cs=='iso646-us';Cs=='us-ascii';Cs=='us';
			  Cs=='ibm367';Cs=='cp367';Cs=='csascii' -> % US-ASCII
    Input;
from_unicode(Input,Cs) when Cs=='iso-10646-utf-1';Cs=='csiso10646utf1' ->
    Input;
from_unicode(Input,Cs) when Cs=='iso_646.basic:1983';Cs=='ref';
			  Cs=='csiso646basic1983' ->
    Input;
from_unicode(Input,Cs) when Cs=='iso_8859-1:1987';Cs=='iso-ir-100';
			  Cs=='iso_8859-1';Cs=='latin1';Cs=='l1';Cs=='ibm819';
			  Cs=='cp819';Cs=='csisolatin1' ->
    Input;
from_unicode(Input,Cs) when Cs=='mnemonic';Cs=='"mnemonic+ascii+38';
			  Cs=='mnem';Cs=='"mnemonic+ascii+8200' ->
    to_mnemonic(Input);
from_unicode(Input,Cs) when Cs=='iso-10646-ucs-2';Cs=='csunicode' ->
    to_ucs2be(Input); % Guess byteorder
from_unicode(Input,Cs) when Cs=='iso-10646-ucs-4';Cs=='csucs4' ->
    to_ucs4be(Input); % Guess byteorder
from_unicode(Input,Cs) when Cs=='utf-16be';Cs=='utf-16' ->
    to_utf16be(Input);
from_unicode(Input,'utf-16le') ->
    to_utf16le(Input);
from_unicode(Input,'utf-8') ->
    to_utf8(Input);
from_unicode(Input,'gsm_0338') ->
    to_gsm0338(Input);
from_unicode(Input,Charset) ->
    ucs_data:from_unicode(Input,Charset).
...
getMIB(Charset) ->
    ucs_data:getMIB(Charset).

getCharset(MIBnum) ->
    ucs_data:getCharset(MIBnum).
