...
xml(Data)->
    {ok, Pid} = wbxml_sup:start_child(),
    gen_server:call(Pid, {decode, Data}).

init([])->
    {ok, #state{}}.

handle_call({decode, Data}, _From, State)->
...
encode(Content,_Accept,ExtCharset,DTDRules) ->
...
lookup_attributelist(_Name,[]) ->
    undefined;
lookup_attributelist(Name,[#xmlAttribute{name=Name,value=Val}|_Attrlist]) ->
    list_to_atom(string:to_lower(Val));
lookup_attributelist(Name,[_H|Attrlist]) ->
...
% ATTRIBUTE MODE
attribute_mode(_Module,[?WBXML_END|Content],_,_,_,Result) ->
...
attribute_mode(_,[],_,_,_,_Result) ->
    {error,premature_end}.
...
global_wbxml_code(?WBXML_OPAQUE,Content,_StrTbl,_) ->
    ...
    {Tail,Str};
global_wbxml_code(Code,Content,_StrTbl,_) ->
